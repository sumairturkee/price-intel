(This is the big one — copy ALL of it carefully)
'use client';
...
{row.sel&&<span style={{color:'#000',fontSize:9,fontWeight:900}}>✓</span>}
</div>
...
onFocus={e=>e.target.style.borderColor=C.accent} onBlur={e=>e.target.style.borderColor='transparent'}/>
<div onClick={()=>setDraft(d=>d.filter((,idx)=>idx!==i))} style={{color:C.muted,cursor:'pointer',fontSize:13,textAlign:'center'}}
...
if(sortCol==='model')return sortDira.model.localeCompare(b.model);
if(sortCol==='variant')return sortDira.variant.localeCompare(b.variant);
...
higher:tableRows.filter(r=>{const c=cheapest(r);return c&&r.myPrice&&(r.myPrice-c)/c100>3}).length,
lower:tableRows.filter(r=>{const c=cheapest(r);return c&&r.myPrice&&(r.myPrice-c)/c100<-3}).length,
};
...
if(col==='model'){if(editVal.trim())await setOv(key,'modelName',editVal.trim());}
